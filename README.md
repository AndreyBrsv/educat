...
## Содержание
1. [Deploy на локальном сервере Jetty](#Deploy-на-локальном-сервере-Jetty)
    - [Добавить конфигурацию Maven](#Добавить-конфигурацию-Maven)
    - [Добавить конфигурацию Jetty](#Добавить-конфигурацию-Jetty)
2. [Соглашения именования и правила написания скриптов](#Соглашения-именования-и-правила-написания-скриптов)
    - [Названия таблиц](#Названия-таблиц)
    - [Название суррогатного ключа](#Название-суррогатного-ключа)
    - [Название внешнего ключа](#Название-внешнего-ключа)
    - [Последовательности](#Последовательности)
    - [Триггеры](#Триггеры)
    - [Индексы](#Индексы)
    - [Комментарии](#Комментарии)
3. [Спецификация формата статей v0.0.1](#Спецификация-формата-статей(v0.0.1))
    - [Элементы оформления статьи](#Элементы-оформления-статьи)
        - [Блок изображения](#Блок-изображения)
        - [Блок цитаты](#Блок-цитаты)
        - [Блок акцента](#Блок-акцента)
        - [Блок кода](#Блок-кода)
    - [Пример статьи, написанной в формате Json](#Пример-статьи,-написанной-в-формате-Json)

## Deploy на локальном сервере Jetty

### Добавить конфигурацию Maven
...
В выпадающем списке выбрать Maven.

- После открытия нового окна в поле name вводим, к примеру Maven(clean/install).

- Далее, на против поля Working directory справа есть иконка папочки с синим квадратом,
нажимаем на нее и выбираем educat.

- В строку Command line вводим:clean install

- Жмем "Ок".

- Выбираем его из списка конфигураций.

- Выполняем его нажав зеленую кнопку play.

После этого происходит генерация .war файла нашего веб приложения. Теперь каждый раз,
когда на сервере происходят изменения, достаточно выбрать эту конфигурацию и нажать play.
Maven отчистит старые файлы и сгенерирует новый .war (Web Application Resource), который, затем, будет автоматически разворачиваться на сервере при каждом запуске.


### Добавить конфигурацию Jetty

Далее нужно установить Jetty сервер на локальную машину, например,
командой:
...
либо его можно скачать [отсюда](https://www.eclipse.org/jetty/download.html) и распаковать в любую удобную папку.
...
наш свежеустановленный Jetty(если в выпадающем списке он не отображается, то выбрать папку, куда Jetty был распакован). 
- Остальные настройки в этом окне пока не трограем.
- Внизу над копкой "OK" будет гореть кнопка "Fix" с красной лампочкой, ее нужно будет нажать два раза, но перед нажатием читай дальше.
- Первое нажатие подтягивает недостающую библиотеку.
- Второе нажатие приведет к выбору артефакта. Артефакт - это тот самый .war, который генерирует maven. Выбираем educat-web:war exploded.

- После этого, нажимаем "ОК".

- Из списка конфигураций выбираем Educat-Jetty (ну или как вы его там обозвали) и нажимаем кнопку жучка. Если все прошло хорошо, то должна открыться главная страница в браузере.
...
## Спецификация формата статей(v0.0.1)
НЕЕЕЕт все хуйня давай по новой, все будем делать на компонентах;
чтобы их можно было засунуть в массив и инициализировать из в виде вьюшек
### Элементы оформления статьи
#### Блок текста
```json
{
  "plainText" : "Какой-то обычный текст, который нужно отобразить" 
}
```

##### Акценты слов внутри текстового блока
bold - жирный $b{text}$, 
italic - наклонный $i{text}$, 
underline - подчеркнутый $u{text}$, 
crossed out - зачеркнутый $c{text}$,
linked - ссылка $l{text}$
в случае жирной ссылки: $bl{text}$, 
жирная зачеркнутая наклонная ссылка "$bcil{text}$"
```json
{
  "plainText" : "Какой-то $b{жирный}$ текст, который нужно отобразить" 
}
```
##### Формулы Latex внутри текстового блока
```json
{
  "formula" : "{f(x) = \frac{A_0}{2} + \sum \limits_{n=1}^{\infty} A_n \cos \left( \frac{2 n \pi x}{\nu} - \alpha_n \right) }",
}
```
#### Блок изображения
```json
...
```json
{
  "quote" : "Текст, на которы нужно наложить стиль цитата"
}
```
#### Блок акцента
```json
{
  "accent" : {
      "colorSchema" : "Цветовая схема акцента",
      "header" : "Заголовок акцента",
      "content" : "Текстовое содержимое акцента",
      "additionalInfo" : "Дополнительная информация"
  }
}
```
#### Блок кода
Я очень хочу верить, что мы сможем реализовать отрисовку данного блока на клиенте.
```json
{
  "code" : {
      "colorSchema" : "Цветовая схема блока кода",
      "language" : "Язык, на котором написано содержимое",
      "content" : "Текстовое содержимое акцента"
  }
}
```

### Пример статьи, написанной в формате Json
```json
{
  "identifier" : "Уникальный идентификатор статьи", 
  "title" : "Заголовок статьи",
  "subtitle" : "Подзаголовок",
  "readingTime" : "Время чтения",
  "publicationDate" : "Дата публикации",
  "authorIdentifier" : "Уникальный иденитифкатор автора статьи",
  "isPublished" : "Признак опубликованности, если false, то контент пустой, ругаемся на клиенте",
  "isBookmarked" : "Добавлена ли в закладки данным пользователем",
  "likes" : "Количество мне нравится",
